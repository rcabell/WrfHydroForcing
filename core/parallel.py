...
class MpiConfig:
    ...
    def merge_slabs_gatherv(self, local_slab, options):
        if self.counts is None:
            ...
            self.width = global_shapes[1]

            # check that all slabes are the same width and sum the number of rows
            self.total_rows = 0
            for i in range(0,self.size):
                self.total_rows += global_shapes[2*i]
                if global_shapes[(2*i)+1] != self.width:
            ...
            # generate counts
            self.counts = [ global_shapes[i*2] * global_shapes[(i*2)+1]
                       for i in range(0,self.size)]

            #generate offsets:
            self.offsets = [0]
            for i in range(0, len(self.counts) - 1):
                self.offsets.append(self.offsets[i] + self.counts[i])
        ...
        if self.rank == 0:
            recvbuf = np.empty([self.total_rows, self.width], local_slab.dtype)
        else:
            recvbuf = None
        ...
        try:
            self.comm.Gatherv(sendbuf=local_slab, recvbuf=[recvbuf, self.counts, self.offsets, data_type], root=0)
        except:
